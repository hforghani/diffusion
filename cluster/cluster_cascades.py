...
def heat_map(mat, min_size, max_size, clust_count, method='spectral'):
    fig, ax = plt.subplots()
    # im = ax.imshow(mat)
    # plt.show()
    file_name = os.path.join(BASEPATH, 'data', 'clusters',
                             f'{DB_NAME}-{min_size}to{max_size}-{method}-{clust_count}.png')
    ...
    def show_clusters(clust_num, min_size, max_size, depth):
        ...
        file_name = os.path.join(BASEPATH, 'data', 'clusters',
                                 f'{DB_NAME}-{min_size}to{max_size}-{method}-{clust_count}')
