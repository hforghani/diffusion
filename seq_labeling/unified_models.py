...
class UnifiedMRFModel(DiffusionModel):
    ...
    def fit(self, train_set, train_trees, project, multi_processed=False, eco=False, iterations=None, **kwargs):
        ...
        logger.debug('number of not-none indexes = %d', len(not_none_indexes))
        ...
        for it in range(max_iterations):
            with Timer('iteration'):
                ...
                # psi = self.__run_hugin(junction_tree, self.__train_max_depth, lambdaa, pred_indexes)

                if multi_processed:
                    gradients = self.__likelihood_grad_mp(not_none_indexes, child_indexes, data_size, feature_sums,
                                                          lambdaa, pred_indexes, self.c2, self.gibbs_samples_num)
                else:
        ...
        # self.__psi = self.__run_hugin(junction_tree, self.__train_max_depth, lambdaa, pred_indexes)
    ...
    def __likelihood_grad_mp(self, not_none_indexes, child_indexes, data_size, feature_sums, lambdaa, pred_indexes, c2,
                             gibbs_samples_num):
        num = len(not_none_indexes)
        indexes = not_none_indexes.copy()
        shuffle(indexes)
        bunch_size = min(100, math.ceil(num / settings.TRAIN_WORKERS))
        indexes_bunches = [indexes[i:i + bunch_size] for i in range(0, num, bunch_size)]
        ...
        grad = list(chain(*grad_bunches))
        ind_to_grad = dict(zip(indexes, grad))
        return [ind_to_grad[ind] for ind in not_none_indexes]
                ...
                for thr in threshold:
                    ...
                    if child_id not in thr_active_ids:
                        ...
                        if new_active_ids & set(parents):
                            parent_id, pred = self.__predict(thr, child_id, trees[thr], parent_states,
                                                             new_active_ids, parents, last_pred)
                            last_pred = pred
                            if parent_id:
    ...
    def __likelihood_grad(cls, node_index, pred_indexes, child_indexes, data_size, feature_sum, lambdaa, c2,
        ...
        grad = feature_sum - data_size * grad_ln_z
    ...
    def __predict(self, thr, node_id, tree, parent_states, new_active_ids, parents, last_pred):
        if last_pred is not None and np.array_equal(parent_states, last_pred.parent_states):
            ...
            prob = self.__act_prob(node_index, parent_states)
    ...
    def __feature(value: typing.Union[int, np.ndarray], pred_num: int):
        ...
        if isinstance(value, int):
            ...
            if value >= 2 ** (pred_num + 1):
                f = np.array([d == '1' for d in f'{bin(value % (2 ** (pred_num + 1)))[2:]:0>{pred_num + 1}}']).astype(
                    float)
            else:
                f = np.zeros(pred_num + 1)
            # logger.debug('feature = %s', arr_to_str(f))
        elif isinstance(value, np.ndarray):
            f = value[1:].astype(float) if value[0] else np.zeros(pred_num + 1)
        else:
            raise TypeError('value must be an integer or numpy.ndarray')
        return f

    def __act_prob(self, node_index, pred_states):
        f = np.concatenate((np.array([False]), pred_states))
        dot = self.__lambda[node_index].dot(f)
    ...
    def __grad_ln_z(cls, node_index, pred_indexes, child_indexes, lambdaa, gibbs_samples_num):
        ...
        with timers[0]:
            ...
            samples_sum = cls.__gibbs_samples_sum(i, gibbs_samples_num, pred_indexes, child_indexes, lambdaa)
        ...
        with timers[4]:
            ...
            # grad_ln_z = np.mean(features, axis=0)
            grad_ln_z = samples_sum / gibbs_samples_num
    ...
    def __gibbs_samples_sum(cls, node_index, samples_num, pred_indexes, child_indexes, lambdaa):
        ...
        # sample = np.array([rows[n] for n in np.random.randint(0, 3, nodes_num)])
        # logger.debug('sample.T = \n%s', two_d_arr_to_str(sample.T))
        node_pred_indexes = pred_indexes[node_index]
        node_pred_num = len(node_pred_indexes)
        node_feat = cls.__feat_i_by_sample(sample, node_index, node_pred_indexes)
        samples_sum = node_feat.copy()
        variables = [(i, t) for i in range(nodes_num) for t in range(2)]
        count = 1
        ...
        while count < samples_num:
            for var in variables:
                ...
                if (t == 1 and sample[i, 0]) or not pred_indexes[i]:
                    ...
                    # Sample S_i^t form the distribution P(S_i^t | others).
                    sigma_dif = cls.__sigma_dif(sample, var, pred_indexes, child_indexes, lambdaa)
                    prob1 = 1 / (1 + np.exp(-sigma_dif))
                    # logger.debug('sigma_dif = %f', sigma_dif)
                    # logger.debug('prob1 = %f', prob1)
                    ...
                    # Extract the feature of node related to node_index.
                    if i == node_index:
                        if t == 1:
                            node_feat = sample[[node_index] + node_pred_indexes, 0].astype(float) if sample[
                                node_index, 1] else np.zeros(
                                node_pred_num + 1)
                        else:
                            node_feat[0] = float(sample[node_index, t])
                    elif i in node_pred_indexes:
                        node_feat[1 + node_pred_indexes.index(i)] = float(sample[node_index, t])

                # logger.debug('sample.T = \n%s', two_d_arr_to_str(sample.T))
                # samples_sum += cls.__feat_i_by_sample(sample, node_index, node_pred_indexes)
                samples_sum += node_feat
                count += 1
                # if count % 10 ** 5 == 0:
                #     logger.debug('%d%% done', count / samples_num * 100)
                if count >= samples_num:
