...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        try:
            count = 0
            for reshare in orm.Reshare.objects.iterator():
                reshare.reshared_post = reshare.post
                text = self.generate_new_text(reshare.post.text)
                if reshare.datetime >= reshare.reshared_post.datetime:
                    dt = reshare.datetime
                else:
                    dt = reshare.reshared_post.datetime + (reshare.reshared_post.datetime - reshare.datetime)
                post = orm['crud.Post'].objects.create(text=text, author=reshare.user, datetime=dt)
                reshare.post = post
                reshare.save()
                count += 1
                print '%d reshares saved' % count
        except:
    ...
    def backwards(self, orm):
        try:
            count = 0
            for reshare in orm.Reshare.objects.filter(post__isnull=False, reshared_post__isnull=False):
                reshare.datetime = reshare.post.datetime
                reshare.user = reshare.post.author
                reshare.post.delete()
                reshare.post = reshare.reshared_post
                reshare.save()
                count += 1
                print '%d reshares done' % count
        except:
            print(traceback.format_exc())
            raise
