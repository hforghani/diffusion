...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        count = 0
        for meme in orm.Meme.objects.iterator():
            posts = orm.Post.objects.filter(postmeme__meme=meme)
            if posts.exists():
                meme.last_time = posts.latest('datetime').datetime
                meme.save()
            count += 1
            if count % 10 == 0:
                print '%d memes done' % count

    def backwards(self, orm):
        orm.Meme.objects.update(last_time=None)
